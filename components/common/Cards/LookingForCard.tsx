...
const LookingForCard = ({ image, title, link, buttonText }: TLookingForData) => {
  return (
    ...
    <div
      ...
      className="group h-full p-5 relative grid grid-rows-2 transition-transform duration-500 overflow-hidden "
    >
        <Image
      ...
      <Link href={link}>
        <button 
         ...
         className=" absolute bottom-8 left-1/2 transform -translate-x-1/2 uppercase text-center border rounded  text-sm shadow-md border-white border-opacity-30 py-2 px-8 hover:bg-dipalo z-20 text-white transition-all duration-500">
        {buttonText}
        </button>
      </Link>
    </div>
